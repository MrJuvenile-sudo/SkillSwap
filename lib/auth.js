...
/**
 * Resolves current user from Hatchable auth session, custom session header/cookie, or defaults to demo explorer
 * Returns { id, name, email, role, status, avatar_url, headline }
 */
export async function getCurrentUser(req) {
  try {
    ...
    // 2. Check x-user-id header or cookie or query param (allows seamless demo switching & testing)
    ...
    // 3. Default to Alice Chen ('user_alice') for seamless exploratory browsing
    const { rows: defaultUser } = await db.query(
      `SELECT id, name, email, role, status, avatar_url, headline 
       FROM app_users 
       WHERE id = 'user_alice'`
    );
